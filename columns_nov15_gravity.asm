...
col_array: .word 6   # array of the current 3 colours in the column, going from top to bottom 
# (sorta like a dictionary except rudimentary)
# col_array = [pixel1 colour, pixel1 address, pixel2 colour, pixel2 address...] for all 3 pixels
...
prep_draw_col:
    ...
    addi $a1, $zero, 3
    jal set_starting_pixel_location
    jal draw_col # draw the column with the generated colours, and the starting position
    j game_loop
...
respond_to_D:
    ...
    add $t8, $t3, $zero     # initialise copy of column array address
    add $t9, $t1, $zero     # intialise copy of bitmap display address
    
    # loop 3 times for each pixel in the column
    # make the stopping condition
    lw $t7, 20($t8)     # load the last pixel location
    addi $t7, $t7, 4  # final stopping condition, once last pixel has moved over
    loop_start_d:
        beq $t7, $t9, loop_end_d
        lw $t9, 4($t8)      # find position of first pixel in column array, store in $t9
        lw $t5, 28($t2)     # load colour black from colours array
        
        sw $t5, 0($t9)      # erase that first pixel (paint that pixel black)
        lw $t5, 0($t8)      # load colour from column array
        
        addi $t9, $t9, 4    # increment address to move pixel 1 unit right
        sw $t5, 0($t9)      # paint the new pixel at the updated location
        sw $t9, 4($t8)      # store NEW address back in column array
        addi $t8, $t8, 8    # increment the column array location too
        j loop_start_d
    loop_end_d:
    j game_loop   # return 
...
respond_to_A:
    ...
    loop_start_a:
        ...
        addi $t9, $t9, -4    # decrement address to move pixel 1 unit LEFT
...
respond_to_S:
    ...
    # make the stopping condition
    lw $t7, 20($t8)     # load the last pixel location
    addi $t7, $t7, 128  # final stopping condition, once last pixel has moved over
    ...
    loop_start_s:
        beq $t7, $t9, loop_end_s
        
        lw $t5, 0($t8)      # load colour from column array
        
        addi $t9, $t9, 128    # decrement address to move pixel 1 unit Down
...
generate_rand_colours: 
    ...
    addi $t7, $t8, 24 # stopping condition (if alreadying generated 3 pixel colours 
    rand_loop_start:
        beq $t8, $t7, rand_loop_end # stop if done drawing all 3 coloured pixels
        ...
        add $t8, $t8, 8     # increment to next space to store colour
...
draw_col: # function to draw the 3 pixel column, with 3 random colours from the 6 available
    add $t8, $t3, $zero # initialise copy of col array address 
    addi $t7, $t9, 384 # end after new address = starting address + 384 for 3 pixels vertically
    col_loop_start:   
        beq $t7, $t9, col_loop_end # stop if done drawing all 3 coloured pixels
        sw $t9, 4($t8)  # store location of pixels in column address
        
        lw $t5, 0($t8) # load colour from the column array
        sw $t5, 0($t9) # paint pixel with that colour

        addi $t8, $t8, 8     # increment 
        addi $t9, $t9, 128
        j col_loop_start
    col_loop_end:
...
game_loop:
    ...
    addi $t7, $zero, 200
    add $a3, $zero, $zero # intialise incrementer to 0
    updating_loop_start:
    beq $t7, $a3, updating_loop_end #if counter reaches 100, then exit loop
    ...
    li $a0, 5    # number of milliseconds to wait
    syscall    # sleep
    ...
    # go to function to check if collision
        ...
        # if collision w bottom or on top of other gems, 
